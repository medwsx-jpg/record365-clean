    ...
    <>
      <div className="bg-white rounded-xl p-4 shadow-sm border">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-bold text-gray-800">의뢰자</p>
      ...
      <button onClick={handleReset} className="w-full bg-red-50 text-red-500 rounded-xl py-3 text-sm font-medium border border-red-100">
        역할 초기화 (데이터 삭제)
      </button>
    </>
  );
...
function CleanerMyPage() {
  ...
  const handleReset = () => {
    localStorage.removeItem('cleanmatch_role');
    localStorage.removeItem('cleanmatch_requests');
    localStorage.removeItem('cleanmatch_cleaner_profile');
    localStorage.removeItem('cleanmatch_completed_jobs');
    localStorage.removeItem('cleanmatch_training');
    window.location.href = '/clean';
  };
      ...
      <div className="bg-white rounded-xl p-5 border border-gray-200">
        <div className="flex items-center gap-4">
          {profile?.photo ? (
            ...
            <div className="w-16 h-16 rounded-full bg-green-100 flex items-center justify-center text-green-600 font-bold text-xl shrink-0">
              {profile?.name?.charAt(0) || '청'}
            </div>
          )}
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <h2 className="text-lg font-bold text-gray-900 truncate">{profile?.name || '프로필 미등록'}</h2>
              <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full shrink-0 ${grade.bgColor} ${grade.color} border ${grade.borderColor}`}>
                {grade.emoji} {grade.label}
              </span>
            </div>
            {profile?.experience && (
              <p className="text-xs text-gray-500 mt-0.5">{expLabels[profile.experience] || ''}</p>
            )}
...
function BottomNavInline() {
  ...
  const clientTabs = [
    ...
    { label: '청소 의뢰', path: '/clean/client/create',
      icon: (<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" /><line x1="12" y1="8" x2="12" y2="16" /><line x1="8" y1="12" x2="16" y2="12" /></svg>) },
    { label: '채팅', path: '#', disabled: true,
  ...
  const cleanerTabs = [
    { label: '홈', path: '/clean/cleaner',
      icon: (<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z" /><polyline points="9 22 9 12 15 12 15 22" /></svg>) },
    { label: '의뢰 목록', path: '/clean/cleaner',
      icon: (<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" /><line x1="8" y1="8" x2="16" y2="8" /><line x1="8" y1="12" x2="16" y2="12" /><line x1="8" y1="16" x2="12" y2="16" /></svg>) },
    { label: '채팅', path: '#', disabled: true,
      icon: (<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" /></svg>) },
    { label: '마이페이지', path: '/clean/mypage',
      icon: (<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg>) },
